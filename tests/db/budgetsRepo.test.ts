...
describe('budgetsRepo', () => {
  ...
  it('soft-deleted expenses are excluded from spent', async () => {
    const eur = await createAccount({ name: 'A', icon: 'w', color: '#1', currency: 'EUR', initialBalance: 0 })
    await createBudget({ categoryId: 'food', month: '2026-06', amount: 10000, currency: 'EUR' })
    await addExpense(eur.id, 'food', 5000, '2026-06-01')
    await db.transactions.add({ id: crypto.randomUUID(), type: 'expense', amount: 3000, accountId: eur.id, categoryId: 'food', date: '2026-06-05', tags: [], createdAt: 't', updatedAt: 't', deletedAt: 't' })
    const [p] = await budgetProgress('2026-06')
    expect(p.spent).toBe(5000)
  })
  it('transfer rows are excluded from spent even with matching categoryId/month/currency', async () => {
    const eur = await createAccount({ name: 'A', icon: 'w', color: '#1', currency: 'EUR', initialBalance: 0 })
    await createBudget({ categoryId: 'food', month: '2026-06', amount: 10000, currency: 'EUR' })
    await addExpense(eur.id, 'food', 2000, '2026-06-01')
    await db.transactions.add({ id: crypto.randomUUID(), type: 'transfer', transferDirection: 'out', amount: 4000, accountId: eur.id, categoryId: 'food', date: '2026-06-10', tags: [], createdAt: 't', updatedAt: 't' })
    const [p] = await budgetProgress('2026-06')
    expect(p.spent).toBe(2000)
  })
  it('amount 0 yields percent 0 and status ok', async () => {
    await createBudget({ categoryId: 'misc', month: '2026-06', amount: 0, currency: 'EUR' })
    const [p] = await budgetProgress('2026-06')
